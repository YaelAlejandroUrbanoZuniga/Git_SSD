...
Management frontend (React/Vite app at the repo root). The API mirrors the contract
implied by `src/services/*.ts` and `src/types/index.ts` of the frontend, and the seed
reproduces `src/data/*.ts` so the frontend looks identical when pointed at the API
(`http://localhost:3000/api`, matching `src/services/api.config.ts`).
...
npm run seed                  # load the demo dataset from ../src/data/*.ts
...
├── prisma/seed.ts         # imports ../src/data/*.ts directly and decomposes it
├── src/
