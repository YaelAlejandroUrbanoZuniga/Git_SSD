...
notifications — ver §3) implementada y cubierta por 245 tests.
...
```bash
npm test                      # 245 tests: unit (business rules) + integration (HTTP)
npm run typecheck
```
...
supplier created, stage move, blacklist, event created and **event edited** each fan out
one notification per **SSD** user. This is a deliberate, permanent audience decision (whole
SSD team) that mirrors SSD being the sole operational writer — there is no finer
per-role/per-commodity targeting planned. Every call site wraps
the notify in `try/catch` so a notification failure can never roll back or fail the
...
| `Category` | **what happened?** (domain event) | `supplier_created` \| `stage_advanced` \| `blacklisted` \| `event_created` \| `event_updated` |

`Category` was added (`sql/2026-08-07_add_notification_category.sql`) because four of the
five events are `info`, so a panel keyed off the severity alone drew the same blue
...
- `tests/unit/notificationsRules.test.ts` — `notifySsdTeam` writes one row per SSD user and
  ...
  requesting `userId`; cross-user `markNotificationRead` is a 404. Plus the **category**
  round-trip (persisted *alongside* the severity, not instead of it; returned by both
  `listNotifications` and `markNotificationRead`; `null` for pre-column rows, which still
  come back) and the **delete ownership** rule (another user's row and a non-existent id
  are the same 404; one foreign id in a batch deletes **nothing**; an empty selection is a
